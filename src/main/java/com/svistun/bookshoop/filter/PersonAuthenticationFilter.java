...
package com.svistun.bookshoop.filter;
...
public class PersonAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(@NotNull HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        Person userData;
        try {
            userData = new ObjectMapper().readValue(request.getInputStream(), Person.class);
        } catch (IOException e) {
            throw new UsernameNotFoundException("User not found exception");
        }
        UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(userData.getEmail(), userData.getPassword());
